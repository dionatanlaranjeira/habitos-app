...
## Stores globais — Signals (não ChangeNotifier)

Stores de estado global (AuthStore, UserStore, LocaleStore) usam **signals** em vez de ChangeNotifier:
...
class UserStore {
  ...
  // Reage a AuthStore com effect()
  UserStore(..., AuthStore authStore) {
    effect(() {
      final authUser = authStore.currentUser.value;
      if (authUser == null) { user.value = null; return; }
...
- **Provider** (não ChangeNotifierProvider) para expor stores
- **Watch** para consumir signals na UI
- go_router `refreshListenable`: passar o signal (ex: `authStore.currentUser`)
...
mixin LoginVariables {
  final loginAS = asyncSignal<AuthModel>(AsyncData(null));
}
